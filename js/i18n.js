...
en: {
  ...
  'recformat.dspAgc': 'Automatic gain control / auto-leveling — <em>not recommended for acoustic analysis or language-documentation archiving</em>',
  ...
  'recfmt.helpRaw': '<b>Faithful, unprocessed capture.</b> Browsers normally apply automatic gain, noise reduction, echo cancellation, and voice isolation to microphone audio — this app switches them all off, so the lossless formats capture the raw signal with nothing added or removed. (Browsers still differ a little in mic input level — Firefox stays closest to the raw hardware level; the app itself adds no gain.) The only thing that can change the level is the optional recording-level slider, if a researcher turns it on.',
  ...
  'recfmt.helpProcessing': '<b>What each processing option does — and why it\'s off by default.</b><ul><li><b>Auto-gain (auto-leveling):</b> evens out loud and quiet speakers for easier listening — but it continuously changes the gain (pumping, and a loud passage fades), destroying the true dynamics acoustic analysis relies on.</li><li><b>Noise reduction:</b> removes steady background hiss/hum — but it also strips real speech detail (fricatives, breath, quiet sounds) and adds artifacts.</li><li><b>Echo cancellation:</b> built for two-way calls — a one-person recording has no echo to cancel, and it can suppress and color the voice.</li><li><b>Auto-normalize after recording:</b> scales a too-quiet take up to a standard level in one pass (no pumping) — but it <b>edits the saved file</b>, so the recording is no longer the unmodified original.</li></ul>All are <b>off by default</b> because archives and acoustic analysis need the <b>unmodified</b> capture; they\'re useful only for casual or low-bandwidth (e.g. MP3) copies where convenience matters more than fidelity.',
  'recfmt.helpTech': '<b>Technical summary (for your archive).</b> By default this app captures the <b>raw</b> microphone signal — automatic gain, noise reduction, echo cancellation, voice isolation, and normalization are all <b>off</b> — at the device\'s native sample rate (usually 48 kHz), and stores it <b>losslessly and bit-faithfully</b> with no resampling, at the bit depth of your chosen format (24-bit by default). A single-microphone source is stored as <b>mono</b>; genuine stereo is preserved. The only processing applied is whatever you deliberately switch on under "Microphone processing" in Settings.',
...
id: {
  ...
  'recformat.dspAgc': 'Kontrol penguatan otomatis / auto-leveling — <em>tidak disarankan untuk analisis akustik atau pengarsipan dokumentasi bahasa</em>',
  ...
  'recfmt.helpRaw': '<b>Perekaman setia, tanpa pemrosesan.</b> Peramban biasanya menerapkan penguatan otomatis, pengurangan noise, penghapus gema, dan isolasi suara pada audio mikrofon — aplikasi ini mematikan semuanya, jadi format lossless menangkap sinyal mentah tanpa ada yang ditambahkan atau dihilangkan. (Peramban tetap sedikit berbeda dalam level masukan mikrofon — Firefox paling dekat dengan level perangkat keras mentah; aplikasi sendiri tidak menambah penguatan.) Satu-satunya yang bisa mengubah level adalah penggeser level rekaman opsional, jika peneliti mengaktifkannya.',
  ...
  'recfmt.helpProcessing': '<b>Apa yang dilakukan setiap opsi pemrosesan — dan mengapa mati secara default.</b><ul><li><b>Auto-gain (auto-leveling):</b> meratakan pembicara keras dan pelan agar lebih enak didengar — tetapi terus mengubah penguatan (pumping, dan bagian keras memudar), merusak dinamika asli yang dibutuhkan analisis akustik.</li><li><b>Pengurangan noise:</b> menghilangkan desis/dengung latar yang stabil — tetapi juga menghilangkan detail ujaran asli (frikatif, napas, suara pelan) dan menambah artefak.</li><li><b>Penghapusan gema:</b> dibuat untuk panggilan dua arah — rekaman satu orang tidak punya gema untuk dihapus, dan ini bisa menekan serta mewarnai suara.</li><li><b>Normalisasi otomatis setelah merekam:</b> menaikkan rekaman yang terlalu pelan ke level standar dalam satu langkah (tanpa pumping) — tetapi ini <b>mengubah berkas tersimpan</b>, jadi rekaman bukan lagi yang asli tanpa modifikasi.</li></ul>Semuanya <b>mati secara default</b> karena arsip dan analisis akustik memerlukan rekaman <b>tanpa modifikasi</b>; ini hanya berguna untuk salinan santai atau berbandwidth rendah (mis. MP3) di mana kemudahan lebih penting daripada kesetiaan.',
  'recfmt.helpTech': '<b>Ringkasan teknis (untuk arsip Anda).</b> Secara default aplikasi ini menangkap sinyal mikrofon <b>mentah</b> — penguatan otomatis, pengurangan noise, penghapusan gema, isolasi suara, dan normalisasi semuanya <b>mati</b> — pada laju sampel asli perangkat (biasanya 48 kHz), dan menyimpannya secara <b>lossless dan setia-bit</b> tanpa resampling, pada kedalaman bit format pilihan Anda (24-bit secara default). Sumber satu mikrofon disimpan sebagai <b>mono</b>; stereo asli dipertahankan. Satu-satunya pemrosesan yang diterapkan adalah yang Anda aktifkan secara sengaja di "Pemrosesan mikrofon" di Pengaturan.',
