...
>
> **This feature is a modification to `rulingAnts/ose-interlinear-viewer`. Nothing in the
> FlexText Editor suite changes.**
>
...
> |---|---|---|
> | **`OSE:`** | `rulingAnts/ose-interlinear-viewer` | **the only repo that gets code changes** |
> | **`FXE:`** | `rulingAnts/flextext-editor` (this repo) | reference only — read for behaviour, never edited, never imported |
>
...
> §0.3. The only change in this repo is this Markdown file and one corrected backlog line, which
> per `CLAUDE.md` ("PLANNING DOCS DO NOT GET A VERSION BUMP") takes no version bump, no
> `BUILD_TAG`, no `sw.js` touch and no staging round.
...
|---|---|---|
| **0** | Plan lands. Seth answers D1–D13. | Every D has a recorded answer, and `grep -rn 'Users/Seth/GIT' plans/assign-by-upload.md` → **no matches** (the committed local path is replaced by the repo URL). |
...
never a real `<Member>` line, never a real speaker name** — `plans/README.md`'s exclusion list), run
...
| **D12** | Plan doc home | **`ose-interlinear-viewer/plans/`**, once push access exists | It documents a change to that repo. It is committed here for now only because the backlog entry lives here and this session had no push access to the sibling. |
