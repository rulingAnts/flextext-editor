...
console.log('\nan unpaired device can get a text OUT — the first real text had no way');
{
  ...
  ok(/const wants = \{ eaf: kind === 'all' \|\| kind === 'eaf', saymore: false, preview: kind === 'preview', fxpa: kind === 'fxpa' \};/.test(ex)
     && /buildBundleFor\(rec, true, \{ full: true, wants \}\)/.test(ex),
     'built by the SAME bundle a paired device uploads, and only the outputs actually chosen (v609 added .fxpa, v614 the listening page)');
