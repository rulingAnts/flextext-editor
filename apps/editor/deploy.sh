#!/usr/bin/env bash
# The ONE deploy entry point for the flextext-editor Worker. Set BOTH dashboard commands to:
#
#     bash deploy.sh
#
# Branch routing lives HERE, in git:
#   productionWeb  → `npx wrangler deploy`                    → https://app.flextext.app/
#   anything else  → `npx wrangler versions upload
#                      --preview-alias <branch>`              → https://<alias>-flextext-editor.68mh29kgsd.workers.dev
