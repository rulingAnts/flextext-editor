...
| `fxSettingsAudit(filter?)` | researcher panel | The answer to "are ALL settings shared between researchers?" — for each device (own + shared; `filter` matches nickname or id prefix), reads the local Kr snapshot AND the shared desired-lane copy (`readSettingsLane`, the exact bytes another researcher's seat decrypts) and diffs them key-for-key. `PASS` means every key agrees; mismatches print per-device. A device with only one source is reported, not failed — an unconfigured device or a pre-lane snapshot is a state, not a bug. |
