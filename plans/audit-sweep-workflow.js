...
const CTX = [
  ...
  '',
  '2026-08-23 — TWO MORE FIXES HAVE SINCE LANDED (read the CURRENT code; do NOT re-report these as',
  'new findings, but DO judge whether the fixes themselves are sound):',
  ' . member_key removal (DELETE /v1/projects/<id>/members) gained a THIRD ORed arm:',
  "   (project_id='' AND instance_id IN (SELECT instance_id FROM instance WHERE researcher_id=owner)).",
  '   This closes the 2026-08-23 sweep finding — a legacy-\'\' grant on a device that moved to a',
  '   DIFFERENT project than the removal project used to survive. GET /v1/researcher/keys is STILL',
  '   unscoped (open item 1) — that is the defence-in-depth that was NOT added here, on purpose.',
  ' . invite pairing identity: invite.invited_by (new nullable column) now records the researcher who',
  '   MINTED the invite, and pairingIdentity() resolves the pairing-response identity through it',
  '   (COALESCE to the instance owner when NULL). Fixes a member-enrolled device showing the OWNER,',
  '   not the enrolling member. Identity IS returned at pairing on purpose (the accept gate).',
  '',
