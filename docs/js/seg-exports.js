...
${withAudio ? `<script>
(function () {
  // The recording, as base64 in chunks (see previewBlob): each decodes on its own, so the page
...
/* ═══════════════════════════════════════════════════════════════════════════════════════════════
 * THE RECORDING IS NEVER ONE STRING (Seth, 2026-09-07: "Could not build the download: allocation
 * size overflow" from the Audio Segmenter's .fxpa, and "Download failed" from the panel's listening
 * page — the same engine path, on real recordings).
 *
 * Both files embed the recording as base64, and the old assembly made ONE string of it and then
 * JSON.stringify'd or template-literal'd it into the document. MEASURED, 2026-09-07, headless
 * Firefox 155 running this module's own functions on the rig:
 *     120 MB of audio (168M chars of base64): stringify ok, template ok
 *     135 MB            (189M chars):          JSON.stringify → "InternalError: allocation size overflow"
 *                                              and the page template with it (it stringifies the same string)
 *   — while the identical string concatenated, and went into a Blob, without complaint.
 * So Firefox refuses to QUOTE a string above roughly 179M characters (consistent with reserving
 * six output characters per input character, the most \uXXXX escaping could need, against its
 * (1<<30)-2 string ceiling). That is ~134 MB of audio, UNDER the 200 MB conversionCaps ceiling —
 * which is why a real recording that passed the size gate still failed. The other engines' hard
 * string caps are in ENGINE_STRING_MAX; V8's, the lowest, is what bounds a .fxpa at ~402 MB of
 * audio on the READING side (JSON.parse must hold the base64 as one string).
 *
...
export const ENGINE_STRING_MAX = Object.freeze({
  ...
  spidermonkey: 1073741822, // (1 << 30) - 2:  Firefox — "InternalError: allocation size overflow"
  jsc: 2147483647,          // 2^31 - 1:       Safari
});
/* Measured (above): the largest base64 Firefox 155 would JSON.stringify sat between 168M and 189M
 * characters. Kept as the number the measurement supports, for the test that keeps the export
 * path clear of it — not as a limit the export applies, because the export no longer stringifies. */
export const FIREFOX_STRINGIFY_SEEN_OK = 167772160;   // 120 MB of audio
export const FIREFOX_STRINGIFY_SEEN_FAIL = 188743680; // 135 MB of audio

/* A base64 chunk is self-contained when its INPUT length is a multiple of three (3 bytes → 4
 * characters, no padding mid-stream), so the chunks concatenate to exactly the whole encoding and
 * each also decodes on its own. 3 MiB of bytes → 4 MiB of characters per part. */
const B64_CHUNK = 3 * 1024 * 1024;
export async function blobToBase64Parts(blob, chunkBytes = B64_CHUNK) {
  const step = Math.max(3, Math.floor(chunkBytes / 3) * 3);
  const parts = [];
  const encode = (bytes) => {
    let s = '';
    for (let i = 0; i < bytes.length; i += 0x8000) s += String.fromCharCode(...bytes.subarray(i, i + 0x8000));
  ...
  if (typeof blob.slice !== 'function') {
    // A blob-like with only arrayBuffer() (tests hand these in): one whole read, still chunked base64.
    const all = new Uint8Array(await blob.arrayBuffer());
    for (let off = 0; off < all.length; off += step) parts.push(encode(all.subarray(off, off + step)));
    return parts;
  }
  for (let off = 0; off < blob.size; off += step) {
    parts.push(encode(new Uint8Array(await blob.slice(off, Math.min(off + step, blob.size)).arrayBuffer())));
  }
  return parts;
}

/* The audio as base64 parts, from either a Blob (encoded chunk by chunk) or a string that is
 * ALREADY base64 (the paragraph tool holds its recording that way — sliced, never re-encoded; a
...
export async function b64PartsOf(src, chunkBytes = B64_CHUNK) {
  if (typeof src === 'string') {
    ...
    const parts = [];
    for (let i = 0; i < src.length; i += step) parts.push(src.slice(i, i + step));
    return parts.length ? parts : [''];
  }
  return blobToBase64Parts(src, chunkBytes);
}

/* The placeholder a document is stringified WITH and then split around. Plain ASCII with no quote
 * or backslash, so JSON.stringify and a template literal both pass it through untouched — and '@'
 * is outside the base64 alphabet, so the audio itself can never contain it. */
export const B64_MARK = '@@FLEXTEXT-B64-PARTS-9f1c@@';

/* Splice the audio into a finished document. `text` is the JSON or HTML with B64_MARK where the
 * base64 belongs (INSIDE its quotes), `parts` the base64 chunks; `join` goes between parts — '' for
 * a JSON string value, '","' to write a JS array of string literals. The marker must occur exactly
 * once: a document that somehow carried it elsewhere would be spliced at the wrong place, so that
 * case returns null and the callers fall back to the one-string assembly, which is always correct
 * and merely large. */
export function spliceB64(text, parts, { type = 'text/plain', join = '' } = {}) {
  const at = text.indexOf(B64_MARK);
  if (at < 0 || text.indexOf(B64_MARK, at + 1) >= 0) return null;
  const out = [text.slice(0, at)];
  parts.forEach((p, i) => { if (i && join) out.push(join); out.push(p); });
  out.push(text.slice(at + B64_MARK.length));
...
export async function fxpaBlob(doc, opts = {}) {
  ...
  return spliceB64(json, await b64PartsOf(src), { type })
    || new Blob([JSON.stringify(buildFxpa(doc, { ...rest, audio: { ...m, b64: await wholeB64(src) } }))], { type });
}
