/* Service worker for the "Flextext Consent Collector" PWA. Precaches its own thin shell PLUS
...
const CACHE = 'consent-collector-' + VERSION;
const SHELL = [
  './',
  'index.html',
  'consent.webmanifest',
  'icons/consent.svg',
  'icons/consent-192.png',
  'icons/consent-512.png',
  'icons/consent-apple-touch.png',
...
function cleanupOldCaches() {
  // Scope to THIS app's OWN caches only ('consent-collector-*'). Several PWAs share one origin/CacheStorage, so
  // an unscoped `k !== CACHE` would delete the editor's + researcher's complete caches and brick them offline.
  return caches.keys().then(keys => Promise.all(
    keys.filter(k => k !== CACHE && k.startsWith('consent-collector-')).map(k => caches.delete(k))));
}
